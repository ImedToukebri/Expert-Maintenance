        ...
        try {
            for (int i = 0; i < data.length(); i++) {
                ...
                while (keys.hasNext()) {
                    ...
                    // Skip 'client_id' when working with the 'interventions' table
                    if ("interventions".equals(tableName) && "client_id".equals(key)) {
                        continue;
                    }

                    // Add key-value pairs to ContentValues
                    values.put(key, row.optString(key, null));
                }

                // Handle client_id logic separately
                if ("interventions".equals(tableName)) {
                    int siteId = row.optInt("site_id", -1); // Get site_id from the JSON object
                    if (siteId != -1) {
                        ...
                        if (cursor.moveToFirst()) {
                            ...
                            Log.d("DB_INSERT_INFO", "Derived client_id: " + clientId + " for site_id: " + siteId);
                        } else {
                            Log.w("DB_INSERT_WARNING", "No client_id found for site_id: " + siteId);
                        }
                        cursor.close();
                    }
                }
                ...
                if (result == -1) {
                    ...
                    Log.d("DB_INSERT_SUCCESS", "Successfully inserted into " + tableName + ": " + values.toString());
                }
            }
        } catch (Exception e) {
